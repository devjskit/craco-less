...
test("the webpack config is modified correctly with the modifyLessModuleRule option", () => {
  ...
  expect(lessModuleRule.use[0].loader).toContain(`${path.sep}style-loader`);
  expect(lessModuleRule.use[0].options.testOption).toEqual("test-value-development");

  expect(lessModuleRule.use[1].options.modules.getLocalIdent).toEqual("test-deep-clone-development");
...
test("throws an error when react-scripts adds an unknown webpack rule", () => {
  ...
  expect(runTest).toThrowError(
    new RegExp("Found an unhandled loader in the development webpack config: " + "/path/to/unknown-loader/index.js")
  );
});
...
test("throws an error when the sass rule is missing", () => {
  ...
  expect(runTest).toThrowError(
    new RegExp("Can't find the webpack rule to match scss/sass files in the " + "development webpack config!")
  );
});
...
test("throws an error when the sass module rule is missing", () => {
  ...
  expect(runTest).toThrowError(
    new RegExp("Can't find the webpack rule to match scss/sass module files in the " + "development webpack config!")
  );
});
