...
const { applyJestConfigPlugins } = require("@craco/craco/dist/lib/features/plugins");
...
test("the jest config is modified correctly", () => {
  ...
  expect(moduleNameMapper["^.+\\.module\\.(css|less|sass|scss)$"]).toEqual("identity-obj-proxy");

  const transformIgnorePatterns = jestConfig.transformIgnorePatterns;
  expect(transformIgnorePatterns[1]).toEqual("^.+\\.module\\.(css|less|sass|scss)$");
});
...
test("throws an error when we can't find CSS Modules pattern under moduleNameMapper in the jest config", () => {
  ...
  expect(runTest).toThrowError(/^Can't find CSS Modules pattern under moduleNameMapper in the test jest config!/);
});
