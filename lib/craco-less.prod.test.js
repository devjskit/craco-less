...
test("the webpack config is modified correctly without any options", () => {
  ...
  const lessRule = oneOfRule.oneOf.find(r => r.test && r.test.toString() === "/\\.less$/");
  expect(lessRule).not.toBeUndefined();
  expect(lessRule.use[0].loader).toContain(`${path.sep}mini-css-extract-plugin`);
  ...
  const lessModuleRule = oneOfRule.oneOf.find(r => r.test && r.test.toString() === "/\\.module\\.less$/");
  expect(lessModuleRule).not.toBeUndefined();
  expect(lessModuleRule.use[0].loader).toContain(`${path.sep}mini-css-extract-plugin`);
  ...
  expect(lessModuleRule.use[2].options.postcssOptions.plugins).not.toBeUndefined();

  expect(lessModuleRule.use[3].loader).toContain(`${path.sep}resolve-url-loader`);
...
test("the webpack config is modified correctly without any options on Windows", () => {
  ...
  const replaceSlashesInLoader = rule => {
    if (typeof rule === "string") {
      ...
      if (typeof rule.loader === "string") {
        ...
        if (rule.loader.includes(`${path.sep}file-loader${path.sep}`)) return rule;
  ...
  webpackConfig.module.rules[1].oneOf = webpackConfig.module.rules[1].oneOf.map(replaceSlashesInLoader);
...
test("the webpack config is modified correctly with all loader options", () => {
  ...
  expect(lessModuleRule.use[2].loader).toContain(`${path.sep}postcss-loader`);
  expect(lessModuleRule.use[2].options.postcssOptions.ident).toEqual("test-ident");
  expect(lessModuleRule.use[2].options.postcssOptions.plugins).not.toBeUndefined();
...
test("the webpack config is modified correctly with the modifyLessModuleRule option", () => {
    ...
    plugins: [
      {
        ...
        options: {
          modifyLessModuleRule: (rule, context) => {
            if (context.env === "production") {
              rule.use[0].options.testOption = "test-value-production";
              rule.use[1].options.modules.getLocalIdent = "test-deep-clone-production";
            } else {
              rule.use[0].options.testOption = "test-value-development";
              rule.use[1].options.modules.getLocalIdent = "test-deep-clone-development";
            }
            return rule;
          },
        },
  ...
  expect(lessModuleRule.use[0].loader).toContain(`${path.sep}mini-css-extract-plugin`);
  expect(lessModuleRule.use[0].options.testOption).toEqual("test-value-production");

  expect(lessModuleRule.use[1].options.modules.getLocalIdent).toEqual("test-deep-clone-production");
  ...
  expect(sassModuleRule.use[1].options.modules.getLocalIdent).toEqual(getCSSModuleLocalIdent);
});
...
test("throws an error when react-scripts adds an unknown webpack rule", () => {
  ...
  expect(runTest).toThrowError(
    new RegExp("Found an unhandled loader in the production webpack config: " + "/path/to/unknown-loader/index.js")
  );
});
...
test("throws an error when the sass rule is missing", () => {
  ...
  expect(runTest).toThrowError(
    new RegExp("Can't find the webpack rule to match scss/sass files in the " + "production webpack config!")
  );
});
...
test("throws an error when the sass module rule is missing", () => {
  ...
  oneOfRule.oneOf = oneOfRule.oneOf.filter(rule => !matchSassModuleRule(rule));
  ...
  expect(runTest).toThrowError(
    new RegExp("Can't find the webpack rule to match scss/sass module files in the " + "production webpack config!")
  );
});
