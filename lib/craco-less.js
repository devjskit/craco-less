...
const hasLoader = (loaderName, ruleLoader) => loaderRegexMap[loaderName].test(ruleLoader);
...
const overrideWebpackConfig = ({ context, webpackConfig, pluginOptions }) => {
  ...
  const createLessRule = ({ baseRule, overrideRule }) => {
    ...
    loaders.forEach(ruleOrLoader => {
      ...
      if ((context.env === "development" || context.env === "test") && hasLoader("style-loader", rule.loader)) {
        lessRule.use.push({
          loader: rule.loader,
          options: {
            ...rule.options,
      ...
      } else if (context.env === "production" && hasLoader("mini-css-extract-plugin", rule.loader)) {
        lessRule.use.push({
          loader: rule.loader,
          options: {
            ...rule.options,
...
const overrideJestConfig = ({ context, jestConfig }) => {
  ...
  const cssModulesPattern = Object.keys(moduleNameMapper).find(p => p.match(/\\\.module\\\.\(.*?css.*?\)/));
  ...
  moduleNameMapper[cssModulesPattern.replace("css", "css|less")] = moduleNameMapper[cssModulesPattern];
  delete moduleNameMapper[cssModulesPattern];

  const transformIgnorePatterns = jestConfig.transformIgnorePatterns;
  const cssModulesPatternIndex = transformIgnorePatterns.findIndex(p => p.match(/\\\.module\\\.\(.*?css.*?\)/));
  ...
  transformIgnorePatterns[cssModulesPatternIndex] = transformIgnorePatterns[cssModulesPatternIndex].replace(
    "css",
    "css|less"
  );
